...
export default function BookingWidget({ facility, user }) {
    // Mock Data
    const mockCourts = [
        { id: 1, name: 'Court 1 (Indoor)', price_per_hour: 250.00 },
        { id: 2, name: 'Court 2 (Outdoor)', price_per_hour: 200.00 }
    ];
    ...
    const price = selectedCourt ? selectedCourt.price_per_hour : 0;

    return (
        <div className="bg-[#10221C] p-6 sm:p-8 rounded-3xl shadow-2xl shadow-[#10221C]/40 border border-white/10 sticky top-10">
            <h3 className="text-2xl font-bold text-white mb-1">Book a Court</h3>
            <p className="text-sm text-gray-400 mb-6">Select a date and time to reserve.</p>
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <div className="relative">
                        <select 
                            ...
                            className="block w-full pl-4 pr-10 py-4 text-base font-medium text-white border-2 border-white/10 bg-white/5 focus:outline-none focus:ring-0 focus:border-[#D6FF3F] rounded-xl appearance-none transition hover:border-white/20 cursor-pointer"
                        >
                            {mockCourts.map(court => (
                                <option key={court.id} value={court.id} className="text-gray-900">
                                    {court.name} - ₱{court.price_per_hour.toFixed(2)}/hr
                                </option>
                            ))}
                        </select>
                ...
                {/* Order Summary */}
                {selectedTimeSlot && (
                    <div className="bg-[#1A332B] rounded-2xl p-5 mt-8 border border-white/5">
                ...
                {user ? (
                    <button 
                        ...
                        className={`w-full font-black text-lg py-4 px-4 rounded-xl transition duration-300 shadow-xl shadow-[#D6FF3F]/20 ${selectedTimeSlot ? 'bg-[#D6FF3F] hover:bg-[#c4ec39] text-[#10221C] hover:-translate-y-1' : 'bg-gray-600 text-gray-400 cursor-not-allowed opacity-50'}`}
                    >
                        Confirm Booking
                    </button>
                ) : (
